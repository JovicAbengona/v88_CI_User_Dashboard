            ...
            <div class="row mt-3">
                <div class="col-lg-6">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form action="<?= base_url(); ?>editUserProfile/<?= $id; ?>" method="POST">
                ...
                <div class="col-lg-6">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form action="<?= base_url(); ?>changePasswordProfile/<?= $id ?>" method="POST">
                                <input type="hidden" name="<?=$this->security->get_csrf_token_name();?>" value="<?=$this->security->get_csrf_hash();?>" />
                ...
                <div class="col-lg-12 mt-3 mb-5">
                    <div class="card">
                        <div class="card-header">
                            <ul class="nav nav-tabs card-header-tabs">
                                <li class="nav-item">
                                    <span class="nav-link active" aria-current="true">Edit Description</span>
                                </li>
                            </ul>
                        </div>
                        <div class="card-body">
                            <form action="<?= base_url(); ?>changeProfileDescription/<?= $this->session->userdata("profile_id") ?>" method="POST">
                                <input type="hidden" name="<?=$this->security->get_csrf_token_name();?>" value="<?=$this->security->get_csrf_hash();?>" />
                                <?php if($this->session->flashdata("description_message") != NULL) { echo $this->session->flashdata("description_message"); } ?>
                                <div class="mb-3">
                                    <textarea name="description" id="description" class="form-control no-resize" rows="5"><?= $description ?></textarea>
                                    <?php if($this->session->flashdata("description_errors") != NULL) { echo $this->session->flashdata("description_errors")["edit_description_error"]; } ?>
                                </div>
                                <?php $this->session->set_flashdata("description_errors", NULL); ?>
                                <?php $this->session->set_flashdata("description_message", NULL); ?>
                                <button type="submit" class="btn btn-success">Save</button>
                            </form>
                        </div>
                    </div>
